...
- Require every incident to include `region`, `feature`, `service`, and `detection` labels.
- Use labels such as `region`, `feature`, `service`, `detection`, `root_cause`, `product_category`, and `client_impact`.
- Provide an incident-generation script or runbook for test events.
- Use `skills/observability/incident-placeholder-template/SKILL.md` only for explicitly requested placeholder incidents; that template bypasses the normal `feature` and `service` label requirement but keeps `detection`.
