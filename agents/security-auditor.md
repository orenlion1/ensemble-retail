...
- **Invoke via:** parallel fan-out with `code-reviewer` and `test-engineer` on large cross-cutting changes (see `AGENTS.md`).
- **Do not invoke from another persona.** If `code-reviewer` flags something that warrants a deeper security pass, the user or main agent initiates that pass — not the reviewer.
