...
- **Invoke via:** parallel fan-out with `code-reviewer` and `security-auditor` on large cross-cutting changes (see `AGENTS.md`), or when following `test-driven-development` during implementation.
- **Do not invoke from another persona.** Recommendations to add tests belong in your report; the user or main agent decides when to act on them.
