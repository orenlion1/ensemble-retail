...
resource "aws_cloudfront_response_headers_policy" "security" {
  ...
  security_headers_config {
    ...
    content_security_policy {
      content_security_policy = "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' https: data:; connect-src 'self' https://${var.api_origin_domain_name} https://${var.cognito_domain_prefix}.auth.${var.aws_region}.amazoncognito.com https://faro-collector-prod-us-east-3.grafana.net; font-src 'self'; frame-ancestors 'none'; object-src 'none'; base-uri 'self'; form-action 'self'"
      override                = true
    }
