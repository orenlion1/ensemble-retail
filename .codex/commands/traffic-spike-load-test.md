...
- The default browser-action scenario uses `BROWSER_ACTION_VUS=20`, `BROWSER_ACTION_DURATION=10m`, and `USER_ACTION_TARGET_RPS=2` so every expected Faro user-action family must reach at least 2 events/second.
- The default combined benchmark peaks at 450 VUs. Increase the project quota or lower `BASE_SPIKE_USERS`, `REGIONAL_SHOPPER_VUS`, or `BROWSER_ACTION_VUS` for quota-constrained validation runs.
