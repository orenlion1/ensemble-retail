...
- The default steady API scenario uses `API_REQUEST_RPS=25`, `API_REQUEST_PRE_ALLOCATED_VUS=20`, and `API_REQUEST_MAX_VUS=65` for the requested 25 requests/second protocol load.
- The default steady API scenario weights inventory lighter than cart/account, and spike shopper journeys use `INVENTORY_REQUEST_INTERVAL=3` so inventory-service does not receive catalog reads on every iteration.
