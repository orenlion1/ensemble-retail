...
Secrets and Terraform changes (including `stacks/serverless`) are NOT applied by the automated workflow; run the Terraform stacks locally. As of 2026-07-12 all Terraform lives in the dedicated shared-infrastructure repository [core-infra](https://github.com/orenlion1/core-infra) (clone as a sibling checkout, `../core-infra`); references to `infra/terraform/...` below map to `core-infra/terraform/...`.
...
Use `ensemble-retail.com` as the canonical public domain. Public edge TLS is provisioned with ACM in `us-east-1` for CloudFront and API aliases. The existing Terraform state and AWS resources retain legacy `ensemble-grafana` identifiers; do not rename or recreate them as part of the repository rename. `ensemble-grafana.com` remains a compatibility alias, and `ensemble-service.com` (added 2026-07-12) is a third apex served by the same CloudFront distribution, shared ACM certificate, and API Gateway HTTP API (`api.ensemble-service.com`). All three domains are registered at Namecheap with custom nameservers delegating to their Route 53 hosted zones; each zone's `name_servers` output in core-infra's `terraform/stacks/edge-static` is the value to configure at the registrar.
