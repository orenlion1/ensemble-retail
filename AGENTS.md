...
- After every frontend deployment, run the same k6 browser-action validation against the deployed URL after the CloudFront invalidation or cache refresh completes: `BASE_URL=https://ensemble-retail.com k6 run load-tests/synthetic-browser-actions.js`.
...
- Whenever `.env` `API_TEST_KEY` changes, patch `secret/ensemble-secrets` in namespace `ensemble-grafana`, restart `deployment/cart-service` and `deployment/account-service`, wait for both rollouts, verify the deployed secret hash matches `.env` without adding newline characters, then run the 30-second production regional smoke test: `TEST_DURATION=30s STOREFRONT_BASE_URL=https://ensemble-retail.com API_BASE_URL=https://api.ensemble-retail.com k6 run load-tests/grafana-cloud-20-user-regional.js`.
