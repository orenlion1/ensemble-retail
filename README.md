...
**Token type requirement (resolved 2026-07-04):** `GRAFANA_CLOUD_PROVIDER_ACCESS_TOKEN` must be a **Cloud Access Policy token** created in the Grafana Cloud Portal (grafana.com, org `1786210`) with scopes `integration-management:read`, `integration-management:write`, and `stacks:read` (type them into the Add Scope search box — they are hidden by default). Grafana **instance** service-account tokens and integration ingest tokens are rejected with `401 Unauthorized` by the Cloud Provider API regardless of their permissions, and this token type cannot be created or renewed via `gcx`. To rotate: create the token in the Cloud Portal, verify it returns `200` against `https://cloud-provider-api-prod-us-east-3.grafana.net/api/v2/stacks/1665320/aws/jobs/cloudwatch/ensemble-grafana-rds-cloudwatch` with `curl -H "Authorization: Bearer <token>"`, then `gh secret set GRAFANA_CLOUD_PROVIDER_ACCESS_TOKEN --body "<new-token>"` and re-run the `Terraform Apply` workflow for the `cloudwatch-integration` stack.
