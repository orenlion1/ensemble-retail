...
- Remediation: `inventory-service` now runs 3 pods, caches inventory reads, uses lightweight probe health groups, has a larger pod memory/CPU budget, and the traffic-spike script records `spike_non_json_responses` with a `count==0` threshold instead of crashing on `productsResponse.json()` when an overloaded edge/API path returns HTML.
