...
The checked-in Kubernetes service manifest is pinned to the deployed account `629513454417`, ECR service images, and three replicas per Spring Boot service. If deploying to another AWS account, replace the IRSA role annotations and image registry before applying `infra/k8s/services.yaml`.
